			...
			<div class="row g-3">
				...
				<?php foreach ($projects as $i => $project): ?>
					...
					<div class="col-6 col-sm-4 col-md-3 col-xl-2">
						<div class="card text-center shadow-sm border-0 h-100" style="background-color: <?= $bg ?>;" title="<?= htmlspecialchars($project['name']) ?>">
				...
				<div class="col-6 col-sm-4 col-md-3 col-xl-2">
					<div class="card text-center shadow-sm border border-dashed bg-light h-100" title="<?= t('dashboard.add_project') ?>">
						...
						<div class="card-footer bg-white text-muted small" data-i18n="dashboard.more_projects"><?= t('dashboard.more_projects', [], 'More Projects') ?></div>
					</div>
				</div>
		...
		<div class="col-lg-3 col-12 mt-4 mt-lg-0">
			<div class="bg-white border rounded p-3 shadow-sm" style="font-family: Poppins, sans-serif;">
				<h6 class="fw-bold mb-3">
					...
					<span data-i18n="dashboard.system_info"><?= t('dashboard.system_info', [], 'System Information') ?></span>
				</h6>
				<ul class="list-unstyled small lh-lg mb-0">
					<?php
					$labelMap = [
						'PHP Version'     => 'system.php_version',
						'Server Software' => 'system.server_software',
						'Document Root'   => 'system.document_root',
						'Date'            => 'system.date'
					];
					foreach ($system as $label => $value):
						$tKey = $labelMap[$label] ?? null;
						$translated = $tKey ? t($tKey, [], $label) : $label;
					?>
						<li><strong><?= $translated ?>:</strong> <?= $value ?></li>
					<?php endforeach; ?>
				</ul>
			</div>
		</div>
